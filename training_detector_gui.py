...
class App(tk.Tk):
    ...
    def export_report(self):
        if not self.last_result:
            messagebox.showinfo("안내", "먼저 검사를 실행하세요.")
            return
        path = filedialog.asksaveasfilename(title="리포트 저장", defaultextension=".txt", filetypes=[("Text File", "*.txt"), ("JSON File", "*.json")])
        if not path:
            return
        out = Path(path)
        try:
            ...
            messagebox.showinfo("저장 완료", f"리포트를 저장했습니다.\n{out}")
